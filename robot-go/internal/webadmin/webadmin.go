...
<div class="wrap">
<div class="cards"><div class="card" id="mRobot"><div class="k">Robot</div><div class="v">-</div></div><div class="card" id="mGame"><div class="k">Game port</div><div class="v">-</div></div><div class="card" id="mKey"><div class="k">Key</div><div class="v">-</div></div><div class="card"><div class="k">Auto</div><div class="v" id="mAuto">-</div></div><div class="card"><div class="k">Online / Target</div><div class="v" id="mOnline">-</div></div><div class="card"><div class="k">Store</div><div class="v" id="mStore">-</div></div></div>
<div class="toolbar"><button class="primary" data-key-action="1" onclick="refreshAll()">Refresh</button><button data-key-action="1" onclick="openAutoDialog()">Auto</button><button data-key-action="1" onclick="openOnlineDialog()">Online</button><button data-key-action="1" onclick="runAction('robotsMove')">Move</button><button data-key-action="1" onclick="runAction('robotsShout')">Shout</button><button data-key-action="1" onclick="runAction('robotsStoreAsync')">Store</button><button data-key-action="1" onclick="runAction('robotsLogoutAsync')">Logout</button><button data-key-action="1" class="danger" onclick="openCleanupDialog()">Cleanup</button><button id="keyButton" onclick="openKeyDialog()">Key</button><button data-key-action="1" onclick="openConfigDialog()">Config</button><button data-key-action="1" onclick="loadLog()">Log</button><span class="spacer"></span><label><input id="autoRefresh" type="checkbox" checked onchange="toggleAutoRefresh()"> Auto refresh</label><label>Count <input id="actionCount" type="number" min="1" max="1000" value="1"></label><span class="muted" id="selectedInfo">Selected 0</span></div>
<div class="scheduler"><div class="schedgrid"><div class="scheditem"><div class="k">Policy</div><div class="v" id="sMode">-</div></div><div class="scheditem"><div class="k">Online</div><div class="v" id="sOnline">-</div></div><div class="scheditem"><div class="k">Store</div><div class="v" id="sStore">-</div></div><div class="scheditem"><div class="k">Scale</div><div class="v" id="sScale">-</div></div><div class="scheditem"><div class="k">Pressure</div><div class="v" id="sPressure">-</div></div><div class="scheditem"><div class="k">Release</div><div class="v" id="sRelease">-</div></div></div><div class="schedreason" id="sReason">-</div></div>
<div class="grid"><div class="panel"><div class="panelhead"><h3>Robots</h3><span id="listInfo">-</span></div><div class="tablebox"><table id="tbl"><thead><tr><th><input type="checkbox" id="checkAll" onchange="toggleAll(this.checked)"></th><th>UID</th><th>CID</th><th>Name</th><th>Account</th><th>State</th><th>Type</th><th>Level</th><th>Job/Grow</th><th>Village</th><th>Area</th><th>X</th><th>Y</th><th>Uptime</th><th>Reconnects</th><th>Store</th></tr></thead><tbody></tbody></table></div></div><div class="panel"><div class="panelhead"><h3>Log</h3><span id="logInfo">/root/config/log_robot</span></div><pre id="log"></pre></div></div></div>
...
<script>
const selected=new Set();let currentRows=[],busy=false,keyBlocked=false,autoRefreshTimer=null;
...
function setBusy(v){busy=v;document.querySelectorAll('button').forEach(b=>b.disabled=v);applyKeyGate()}function applyKeyGate(){document.querySelectorAll('[data-key-action="1"]').forEach(b=>b.disabled=busy||keyBlocked);const kb=byId('keyButton');if(kb)kb.disabled=busy}
...
async function refreshAll(){await guarded('Refresh',async()=>{await Promise.all([refreshStatus(),refreshScheduler(),refreshRobots(),checkGame(),refreshKeypair()]);return 'refreshed '+new Date().toLocaleTimeString()})}
...
async function checkGame(){const x=await fetch('/api/game-port').then(r=>r.json());byId('gameHead').textContent='Game: '+(x.ok?'open':'closed')+' '+(x.addr||'');const el=byId('mGame');el.className='card '+(x.ok?'good':'bad');el.querySelector('.v').textContent=x.ok?'Open '+(x.max_user_num||''):'Closed'}
