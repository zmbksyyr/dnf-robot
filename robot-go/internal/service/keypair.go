...
func EnsureRuntimeKeypair(cfg *config.SysConfig) {
	...
	if pair, _, err := ensureGameKeypair(cfg); err == nil {
		if err := copyKeypairToConfig(cfg.ConfigDir, gamePriv, gamePub, pair); err != nil {
			dnf.LogString(dnf.LogLevelIndispensable, fmt.Sprintf("KEYPAIR_CONFIG_SYNC_FAILED err=%v\n", err))
		}
		return
	} else {
		dnf.LogString(dnf.LogLevelIndispensable, fmt.Sprintf("KEYPAIR_GAME_INVALID private=%s public=%s err=%v action=blocked_until_release_default\n", gamePriv, gamePub, err))
	}
}

func (m *RobotManager) ReleaseDefaultKeypair() (KeypairStatus, error) {
	if err := writeDefaultKeypairToGameAndConfig(m.cfg); err != nil {
		return BuildKeypairStatus(m.cfg), err
	}
	cfgPriv, _ := keypairConfigPaths(m.cfg.ConfigDir)
	if err := InitPrivateKey(cfgPriv); err != nil {
		return BuildKeypairStatus(m.cfg), err
	}
	dnf.SetRSAKey(GetRSAKey())
	st := BuildKeypairStatus(m.cfg)
	st.Loaded = GetRSAKey() != nil
	dnf.LogString(dnf.LogLevelIndispensable, fmt.Sprintf("KEYPAIR_DEFAULT_RELEASED game_private=%s game_public=%s fingerprint=%s\n", st.GamePrivate, st.GamePublic, st.Fingerprint))
	return st, nil
}

func (m *RobotManager) KeypairStatus() KeypairStatus {
	st := BuildKeypairStatus(m.cfg)
	st.Loaded = GetRSAKey() != nil
	return st
}
...
func ensureGameKeypair(cfg *config.SysConfig) (loadedKeypair, string, error) {
	...
	if !pubExists {
		...
		dnf.LogString(dnf.LogLevelIndispensable, fmt.Sprintf("KEYPAIR_PUBLIC_DERIVED game_public=%s source_private=%s\n", gamePub, gamePriv))
