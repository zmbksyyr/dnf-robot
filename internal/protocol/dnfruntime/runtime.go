...
func (rs *RobotSvc) ReturnToCharacterSelect(uid int) bool {
	vo := rs.robot(uid)
	return vo != nil && vo.ReturnToCharacterSelect()
}

func (rs *RobotSvc) AtCharacterSelect(uid int) bool {
	vo := rs.robot(uid)
	return vo != nil && vo.Snapshot().State == dnf.StateSelect
}

func (rs *RobotSvc) CharacterSelectRejected(uid int) bool {
	vo := rs.robot(uid)
	return vo != nil && vo.Snapshot().ReturnSelectRejected
}

func (rs *RobotSvc) ReselectCharacter(uid int) bool {
	vo := rs.robot(uid)
	return vo != nil && vo.ReselectCharacter()
}
...
func (rs *RobotSvc) Online(users []shared.RuntimeOnlineUser) error {
	...
	for _, user := range commandUsers {
		if user.DisjointCost == 0 {
			continue
		}
		// The proven disjoint implementation submitted the login directly to
...
func (rs *RobotSvc) RuntimeStatusMap() map[int]shared.RuntimeStatus {
	...
	for _, vo := range robotMap {
		...
		item := shared.RuntimeStatus{
			...
			StoreDisplayItems:    snap.StoreDisplayItems,
