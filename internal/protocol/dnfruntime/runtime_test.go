...
func TestRobotServiceDispatchesDisjointOnlineWithoutGeneralBacklog(t *testing.T) {
	driver := &recordingDriver{}
	service := &RobotSvc{running: true, table: driver}
	service.cond = sync.NewCond(&service.mu)
	service.msgQueue = append(service.msgQueue, robotMsgEntry{typ: robotCommandMove, move: shared.RuntimeMoveCommand{UID: 17000002}})

	user := shared.RuntimeOnlineUser{UID: 17000001, DisjointCost: 500}
	if err := service.Online([]shared.RuntimeOnlineUser{user}); err != nil {
		t.Fatal(err)
	}

	calls, _ := driver.snapshot()
	if len(calls) != 1 || calls[0].typ != robotCommandOnline || calls[0].online[0] != user {
		t.Fatalf("direct disjoint dispatch = %+v", calls)
	}
	if len(service.msgQueue) != 1 || service.msgQueue[0].typ != robotCommandMove {
		t.Fatalf("general backlog changed = %+v", service.msgQueue)
	}
}
