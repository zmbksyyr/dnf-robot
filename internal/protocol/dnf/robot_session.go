...
const (
	...
	// StateSelect is the authenticated character-selection state reached by
	// the game's normal small logout. The old character has been saved and
	// unloaded, but the account connection remains available for re-selection.
	StateSelect ClientState = 6
)
...
type RobotVo struct {
	...
	storeDisplayRetryPlans [][]StoreInfo
	storeDisplayRetryIndex int
	...
	partyRealtimeCandidate      [4]uint16
	partyRealtimeConfirmations  uint8
	partyRealtimeCandidateAt    time.Time
	partyHumanObserved          bool
	partyRecovery               bool
	partyRecoveryResumePending  bool
	partyRecoveryEpoch          uint64
	partyRecoveryCooldownUntil  time.Time
...
func (r *RobotVo) publishSnapshotUnsafe() RobotSnapshot {
	snapshot := RobotSnapshot{
		...
		StoreDisplayItems:    len(r.LastStoreDisplay),
		...
		ReturnSelectPending:  r.ReturnSelectPending,
		ReturnSelectRejected: r.ReturnSelectRejected,
...
func (r *RobotVo) Load(info UserLoginInfo) {
	...
	r.partyRecoveryCooldownUntil = time.Time{}
...
func (r *RobotVo) closeOutUnsafe() {
	r.logPartyTransportClearedUnsafe("session closed")
	r.stopPartySupervisorUnsafe()
	if r.Conn != nil {
		r.Conn.Close()
		r.Conn = nil
