...
var blowfishInitS = [4][256]uint32{
	{
...
func (b *BlowFishCipher) SetKey(key []byte) error {
	if len(key) < 1 {
	...
	aucLocalKey := make([]byte, 56)
	copy(aucLocalKey, key[:keysize])
	copy(b.P[:], blowfishInitP[:])
	b.S = blowfishInitS
	p := aucLocalKey
	iCount := 0
	for i := 0; i < 18; i++ {
...
func (b *BlowFishCipher) KeySize() int {
	return 56
}

func (b *BlowFishCipher) BlockSize() int {
	return 8
}

func (b *BlowFishCipher) F(x uint32) uint32 {
	return ((b.S[0][byte(x>>24)] + b.S[1][byte(x>>16)]) ^ b.S[2][byte(x>>8)]) + b.S[3][byte(x)]
}

func (b *BlowFishCipher) encryptBlock(block *SBlock) {
	L := block.L
	R := block.R
	L ^= b.P[0]
	for i := 1; i < 17; i += 2 {
...
func (b *BlowFishCipher) decryptBlock(block *SBlock) {
	L := block.L
	R := block.R
	L ^= b.P[17]
	for i := 16; i > 1; i -= 2 {
...
func bytesToBlock(p []byte) SBlock {
	return SBlock{
		L: uint32(p[0])<<24 | uint32(p[1])<<16 | uint32(p[2])<<8 | uint32(p[3]),
		R: uint32(p[4])<<24 | uint32(p[5])<<16 | uint32(p[6])<<8 | uint32(p[7]),
	}
}

func blockToBytes(b SBlock, p []byte) {
	p[0] = byte(b.L >> 24)
	p[1] = byte(b.L >> 16)
	p[2] = byte(b.L >> 8)
	p[3] = byte(b.L)
...
func (b *BlowFishCipher) Encrypt(data []byte) ([]byte, error) {
	if !b.init {
	...
	for i := 0; i < len(data); i += blockSize {
		block := bytesToBlock(data[i : i+blockSize])
		b.encryptBlock(&block)
		blockToBytes(block, out[i:i+blockSize])
	}
	return out, nil
}

func (b *BlowFishCipher) Decrypt(data []byte) ([]byte, error) {
	if !b.init {
		return nil, ErrNotInitialized
	}
	blockSize := b.BlockSize()
