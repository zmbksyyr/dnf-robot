...
func TestLogCloseFlushesBufferedRecords(t *testing.T) {
	...
	LogString("buffered-close-record\n")
	LogClose()
...
func TestLogRotationKeepsConfiguredFileBounds(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		LogString("bounded-record-abcdefghijklmnopqrstuvwxyz\n")
	}
	LogClose()
	...
	for writer := 0; writer < writers; writer++ {
		...
		go func(writer int) {
			...
			for record := 0; record < records; record++ {
				LogString(fmt.Sprintf("writer=%d record=%d\n", writer, record))
			}
		}(writer)
	}
	close(start)
