...
func TestExecuteActionsAllowsAuctionSuccessWithZeroSequenceID(t *testing.T) {
	ok := true
	app := testApp(t)
	...
	if !queueContains(app.auctionQueue, 1001) || queueContains(app.auctionSpecialQueue, 1001) || queueContains(app.auctionRejected, 1001) {
		t.Fatalf("accepted auction register should not enter rejected queue; normal=%v special=%v rejected=%v", app.auctionQueue, app.auctionSpecialQueue, app.auctionRejected)
	}
	if _, ok := app.auctionRejectedMeta[1001]; ok {
		t.Fatalf("accepted auction register retained rejected metadata: %#v", app.auctionRejectedMeta[1001])
	}
}
