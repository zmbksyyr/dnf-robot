...
func loadConfig(configDir string) (Config, string, configLoadStatus, error) {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		backupPath, recoverErr := recoverInvalidConfig(path, cfg)
		if recoverErr != nil {
			return cfg, path, configLoadStatus{}, fmt.Errorf("decode market config: %v; restore defaults: %w", err, recoverErr)
		}
		return cfg, path, configLoadStatus{
			Recovered:  true,
...
func recoverInvalidConfig(path string, cfg Config) (string, error) {
