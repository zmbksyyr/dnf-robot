...
var auctionMemoryPatchSpecs = []auctionMemoryPatchSpec{
	...
	{name: "level_operate_parameter", fallbackAddr: 0x080811d7, expect: 0x46, value: 0x7f, targetOffset: 8, pattern: []byte{0x8b, 0x45, 0x20, 0x8b, 0x40, 0x14, 0x83, 0xf8, 0x00, 0x7e, 0x0a, 0xb8, 0x1b, 0x00, 0x00, 0x00}},
	...
	{name: "level_specific", fallbackAddr: 0x0808292d, expect: 0x46, value: 0x7f, targetOffset: 8, pattern: []byte{0x8b, 0x45, 0x0c, 0x8b, 0x40, 0x0c, 0x83, 0xf8, 0x00, 0x7f, 0x07, 0xb8, 0x01, 0x00, 0x00, 0x00}},
	{name: "level_category_min", fallbackAddr: 0x08083472, expect: 0x46, value: 0x7f, targetOffset: 8, pattern: []byte{0x8b, 0x45, 0x0c, 0x0f, 0xb6, 0x40, 0x09, 0x3c, 0x00, 0x77, 0x0b, 0x8b, 0x45, 0x0c, 0x0f, 0xb6}},
	{name: "level_category_max", fallbackAddr: 0x0808347d, expect: 0x46, value: 0x7f, targetOffset: 8, pattern: []byte{0x8b, 0x45, 0x0c, 0x0f, 0xb6, 0x40, 0x0a, 0x3c, 0x00, 0x76, 0x0a, 0xb8, 0x1b, 0x00, 0x00, 0x00}},
}
...
func (a *App) patchAuctionMemory() (AuctionMemoryPatchResult, error) {
	...
	for _, spec := range auctionMemoryPatchSpecs {
		...
		case spec.expect:
			...
			result.Patched++
		default:
			entry.Message = fmt.Sprintf("unexpected byte 0x%02x at %s; want 0x%02x or 0x%02x", buf[0], entry.Address, spec.expect, spec.value)
			result.Entries = append(result.Entries, entry)
			continue
		}
		if _, err := mem.ReadAt(buf[:], address); err != nil {
...
func patchPatternMatch(window []byte, spec auctionMemoryPatchSpec) bool {
	...
	if window[spec.targetOffset] != spec.expect && window[spec.targetOffset] != spec.value {
		return false
	}
