...
func TestTargetAuctionSelectionUsesExplicitItemIDs(t *testing.T) {
	...
	catalog := map[uint32]catalogItem{
		28237: {ItemID: 28237, Kind: "equipment", Level: 35, Attach: "trade", Slot: "weapon", Rarity: 4, Price: 100},
	...
	if !rows[0].ExplicitTarget {
		t.Fatalf("target row = %#v, want explicit target marker", rows[0])
	}
...
func TestAuctionRejectedQueueCooldownAppliesToFullGapMode(t *testing.T) {
	app := testApp(t)
	app.cfg.Restock.EquipmentQtyMin = 1
	app.cfg.Restock.EquipmentQtyMax = 1
	catalog := map[uint32]catalogItem{
		10001: {ItemID: 10001, Kind: "equipment", Attach: "trade", Slot: "weapon", Price: 100},
	}
	app.auctionQueue = []uint32{10001}
	for i := uint32(1); i <= 120; i++ {
		id := 20000 + i
		catalog[id] = catalogItem{ItemID: id, Kind: "equipment", Attach: "trade", Slot: "weapon", Price: 100}
		app.auctionRejected = append(app.auctionRejected, id)
	}
	app.auctionQueueSource = marketQueueSourcePVFItemInfo

	for round := 1; round < auctionRejectedRetryEvery; round++ {
		selection, err := app.nextAuctionQueueSelection(true, catalog, map[uint32]int{}, 0)
		if err != nil {
			t.Fatal(err)
		}
		if selection.Selected.Normal != 1 || selection.Selected.Rejected != 0 {
			t.Fatalf("round %d selection = %#v, want full normal queue without rejected retries", round, selection.Selected)
		}
	}
	...
	wantRejected := defaultMarketMaxActions / auctionRejectedRetryDivisor
	if selection.Budget.Rejected != wantRejected || selection.Selected.Normal != 1 || selection.Selected.Rejected != wantRejected {
		t.Fatalf("cooldown selection budget=%#v selected=%#v, want normal=1 rejected=%d", selection.Budget, selection.Selected, wantRejected)
	}
}
...
func TestAuctionSpecialQueuePrioritizesPetArtifacts(t *testing.T) {
	...
	_, special := testApp(t).catalogAuctionIDsByType(catalog, nil)
...
func TestMarkAuctionExplicitRejectedMovesID(t *testing.T) {
	...
	app.markAuctionRejected(10075, "explicit_rejected")
