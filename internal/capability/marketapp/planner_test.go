...
func TestMarketRarityFilterUsesConfiguredBlockedDigits(t *testing.T) {
	...
	result = &PlanResult{}
	app.planAuction([]restockRow{
		{ItemID: 1002, SystemPrice: 100, Quantity: 1, StackSize: 1, Enabled: true, ExplicitTarget: true},
	}, catalog, map[uint32]int{}, map[uint32]int{}, result)
...
func TestPlanAuctionEquipmentUsesSingleRecordPrice(t *testing.T) {
	...
	for _, action := range result.Actions {
		...
		if !action.HasEndurance {
			t.Fatalf("equipment has_endurance = false, want true")
		}
...
func TestPlanAuctionEquipmentWithoutDurabilityLeavesEnduranceZero(t *testing.T) {
	app := testApp(t)
	result := &PlanResult{}
	catalog := map[uint32]catalogItem{
	...
	if result.Actions[0].Endurance != 0 {
		t.Fatalf("non-durable equipment endurance = %d, want 0", result.Actions[0].Endurance)
	}
	if result.Actions[0].HasEndurance {
