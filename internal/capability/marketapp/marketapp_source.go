...
func (a *App) targetAuctionRowForID(pvfReady bool, catalog map[uint32]catalogItem, id uint32) (restockRow, bool) {
	if !pvfReady {
		row, ok := a.auctionRowForID(false, catalog, id)
		row.ExplicitTarget = ok
		return row, ok
	}
	item, ok := catalog[id]
	if !ok {
	...
	row, ok := a.explicitTargetAuctionRow(item)
	row.ExplicitTarget = ok
	return row, ok
}
...
func (a *App) explicitTargetAuctionRow(item catalogItem) (restockRow, bool) {
	return a.catalogAuctionRowWithQuality(item, false)
}
...
func (a *App) catalogAuctionRowWithQualityConfig(item catalogItem, applyQualityFilter bool, cfg Config) (restockRow, bool) {
	...
	if applyQualityFilter && !marketListingAllowedWithConfig(item, cfg) {
		return restockRow{}, false
	}
	row := restockRow{
		ItemID:      item.ItemID,
