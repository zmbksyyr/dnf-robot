...
func (a *App) targetAuctionSelection(pvfReady bool, catalog map[uint32]catalogItem, have map[uint32]int, itemIDs []uint32) (auctionQueueSelection, error) {
	...
	for _, id := range itemIDs {
		...
		row, ok := a.targetAuctionRowForID(pvfReady, catalog, id)
		if !ok {
			continue
		}
...
func (a *App) auctionQueueBudgetLocked(maxActions int) auctionQueueBudget {
	if maxActions <= 0 {
		// Full-gap mode removes the normal queue ceiling, but it must not turn
		// rejected-item cooldown into an unlimited retry on every round.
		return auctionQueueBudget{Rejected: a.auctionRejectedRetryBudgetLocked(defaultMarketMaxActions)}
	}
	rejected := a.auctionRejectedRetryBudgetLocked(maxActions)
...
func (a *App) auctionRejectedRetryBudgetLocked(maxActions int) int {
