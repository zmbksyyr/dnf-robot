...
func (b *diagnosticsBuilder) addSkillSection() {
	configDir := b.cfg.ConfigDir
	checks := []diagnosticsCheck{
		skillCatalogCheck(filepath.Join(configDir, "party_skill_catalog.json"), true),
		skillCatalogCheck(filepath.Join(configDir, "pvf_skill_state_catalog.json"), false),
	}
	checks = append(checks, recentLogPatternCheck("recent party skill errors", filepath.Join(configDir, "log_robot"), []string{"PARTY_DUNGEON_SKILL_CATALOG_ERROR", "PARTY_DUNGEON_SKILL_SEND_ERROR", "PARTY_DUNGEON_SKILL_RELAY_ERROR"}))
...
func skillCatalogCheck(path string, whitelist bool) diagnosticsCheck {
	data, err := os.ReadFile(path)
	if err != nil {
	...
	status := diagOK
	msg := "skill catalog is readable"
	if len(entries) == 0 {
		status = diagWarn
		msg = "skill catalog has no entries"
	} else if whitelist && (over70 > 0 || badData > 0) {
		status = diagWarn
		msg = "skill catalog has risky entries"
	}
	return diagnosticsCheck{Name: filepath.Base(path), Status: status, Message: msg, Observed: map[string]interface{}{"path": path, "count": len(entries), "by_job": byJob, "over_level_70": over70, "bad_state_data": badData}}
}
