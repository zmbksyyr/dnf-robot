...
func buildRobotPositionUpdate(updates []robotcap.PositionUpdate) (string, []interface{}) {
	...
	// Runtime positions are authoritative for normal robots. Source-coordinate
	// CAS cannot recover once a server cache and Dummylist have drifted apart.
	...
	query.WriteString(") AS p ON d.UID=p.uid AND d.CID=p.cid SET d.curvill=p.curvill,d.curarea=p.curarea,d.curx=p.curx,d.cury=p.cury WHERE d.function_type='0'")
