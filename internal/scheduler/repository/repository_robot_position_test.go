...
func TestExecuteRobotPositionUpdatesSplitsAtBatchBoundary(t *testing.T) {
	...
	if got := len(calls[0].args); got != robotPositionBatchSize*6 {
		t.Fatalf("first args got %d want %d", got, robotPositionBatchSize*6)
	}
...
func TestBuildRobotPositionUpdateHasIdentityAndRoleGuards(t *testing.T) {
	...
	if strings.Contains(query, "fromvill") || strings.Contains(query, "fromarea") {
		t.Fatalf("query retained stale source-position guards: %s", query)
	}
	if len(args) != 6 {
